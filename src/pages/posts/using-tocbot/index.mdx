...
useEffect 内で tocbot.init() して初期化します。第二引数に空の配列`[]`を渡すことで最初のマウント時以外は useEffect() が実行されないようにします。
また `return () => tocbot.destroy()` は、useEffect のクリーンナップで tocbot.destroy() を呼ぶことを意味します。例えば、他のページに遷移して新しく目次を生成する直前に呼ばれ、前回の tocbot の結果を削除します。削除しないと古い結果が残りつづけ意図しない挙動になることがあります。
...
実際に html に変換されると構造は上記のように`<div class="toc">...</div>`で囲う結果になります。
...
手前味噌ながら、この[ブログの実装]("https://github.com/kimizuy/blog.kimizuy.dev/blob/main/src/styles/toc.css")を紹介します。
