...
useEffect 内で tocbot.init() して初期化します。`return () => tocbot.destroy()`はクリーンナップ関数です。次のレンダー時に`tocbot.init()`する直前に呼ばれ、前回の tocbot の結果を削除します。また第二引数に空の配列`[]`を渡すことで最初のマウント時以外は useEffect() が実行されないようにします。
