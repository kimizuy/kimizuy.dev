...
export const meta = {
  ...
  dates: { published: '2020-08-18' },
  tag: ['next', 'mdx', 'typescript'],
  image: card,
}
...
- [Next.js](https://nextjs.org/)
  ...
  React 製のフレームワーク。[GatsbyJS](https://www.gatsbyjs.com/) と比較してもコントローラブルな点で優位だと思う（フォントを変えるのにも苦労した記憶がある）。一方、ブログサイトを作る点では Gatsby の方が便利なプラグインが多い。Next は v9.5 で動的コンテンツを静的ページで生成できる Incremental Static Regeneration という機能の安定板をリリースしている。この機能のデモは[こちら](https://static-tweet.now.sh/)。ツイートを iframe を使わない静的なコンテンツとして生成することでページの表示速度を向上させている。期待を込めて採用。
...
- [Prism.js](https://prismjs.com/)
  ...
  syntax highlighter。実際に利用しているのは[@mapbox/rehype-prism](https://github.com/mapbox/rehype-prism)。代替候補として[Shiki](https://github.com/shikijs/shiki)を検討している。
